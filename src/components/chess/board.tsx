...
export default function ChessBoard(props: {
  ...
  if (!game.isValidGameState()) {
    return <Notice important={true}>Invalid game position</Notice>;
  }

  const isTileSelected = (row: number, column: number) => {
    return props.highlightedTiles?.some(
        ...
        <div className="flex" key={rowIndex}>
          {row.map((piece, columnIndex) => (
            ...
            >
              ...
              {isTileSelected(rowIndex, columnIndex) && (
                <div className={boardStyle.highlightOrb} />
              )}
            </div>
          ))}
        </div>
